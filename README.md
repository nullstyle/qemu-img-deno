...
| Byte builders   | `./fs`          | `buildGpt`, `buildFat` and `buildTar` as pure functions over bytes. No host tooling, no privileges, nothing spawned.                                                      |
...
| GPT repair      | `./block`       | Reads a GPT back off a disk, says what a `resize` broke, and repairs it in place through `raw` windows — no `sgdisk`, nothing materialized.                               |
...
| `./fs` — `buildGpt`, `buildFat`, `buildTar`      | none                                                                                              |
...
| `resize(…, { shrink: true })`                         | Discards everything past the new end, including a GPT's backup header, and leaves both headers' `LastUsableLBA` past the disk. Repair with `repairGptImage()` from `./block`, which **refuses** when a partition now runs off the end and names the byte size to grow back to. Earlier releases prescribed `sgdisk -e` — a tool this package's target host does not have.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `resize()` GROWING a partitioned image                | Adds space **outside the GPT's stated usable range**, and exits `0`. Measured on Alpine's aarch64 cloud image, `+1G`: the primary header at LBA 1 was untouched, so `AlternateLBA` and `LastUsableLBA` still named sector 503807 — the old end — the backup header stayed stranded there mid-disk, and the new final sector was all zeros. Linux still parsed the table and mounted both partitions, because it reads the primary. A tool that trusts `LastUsableLBA` as read will not place anything in the new space; a table-aware grower such as cloud-utils' `growpart` first relocates the backup header to the disk end (via its sgdisk/sfdisk provider, unmeasured here) and only then extends a partition. Repair the backup header at the new tail — with `repairGptImage()` or growpart — before treating the space as real. |
...
const recipe = defineRecipe({
  ...
  base: { kind: "blank", sizeBytes: 48 * 1024 * 1024 },
  ...
  steps: [{
    ...
    partitions: [{
      label: "EFI",
      type: "esp",
      size: 33 * 1024 * 1024, // whatever holds the tree; plan() checks it
...
| `bytes`  | GPT tables, FAT filesystems               | `qemu-img` only — both are written byte by byte by this package   |
...
### Sharp edges: partition tables (`./block`)

Every resize invalidates a GPT, and nothing in the stack says so. Measured
against qemu-img 11.0.2 and macOS 25.5's own `diskutil`/`gpt(8)`, which share no
code with qemu or with this package.

| Invocation                                                     | What actually happens                                                                                                                                                                                                                                                                       |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `convert -n` into a `raw` window from an all-zero source       | Writes the zeros — it does **not** elide them as a no-op. Both the GPT writer's 127 explicitly-zeroed entry slots and the repair's overwrite of a stranded backup header depend on this, so it is asserted in `deno task smoke:block` rather than assumed.                                  |
| `repairGpt()` on two intact headers that disagree              | **Refused**, with no acknowledgement to opt back in. Either table may be the right one and nothing on the disk says which, so picking a side would silently discard the other.                                                                                                              |
| `repairGpt(…, { acknowledgeDataLoss: true })`                  | Drops an over-hanging entry **whole**. It never shortens one to fit: a clamped entry leaves a filesystem whose superblock still claims the blocks past the new end — a corrupt disk that parses cleanly.                                                                                    |
| A guard inside `resize()` itself                               | **Not added.** `resize()` never opens the image today, and detecting "GPT-bearing" needs a read of LBA 0–1 — so a guard would make every resize do I/O, including the majority of images that carry no partition table at all. The paired operation is `resizeAndRepairGpt()` in `./block`. |
| `diskutil verifyDisk` as a damage oracle                       | Not usable on images this size: it refuses with `The target disk is too small for this operation (-69771)` whether the table is sound or not. `gpt -r show` is the oracle that works — by omission, not by erroring.                                                                        |
| A backup header written as a byte-for-byte copy of the primary | Invalid: `MyLBA`/`AlternateLBA` are swapped and the entry array lives at the tail, so the header CRC differs too. Some tools accept the copy and others reject it, which is what makes this look intermittent. Both writers here build the two headers separately.                          |
...
| A FAT filesystem written into a partition window                        | **Written by this package**, not by `mkfs.fat` and not by qemu's `vvfat`. Three things follow. **(1)** The geometry follows the CONTENT, so an ESP is whatever size the recipe declares — measured: the smallest FAT12 window holding a 300 KB payload is 330240 bytes, and a realistic ESP tree fits a 2124800-byte FAT16 volume. Through 0.2.1 vvfat's geometry was fixed and content-independent at 528450048 bytes (FAT16) or 32997888 (FAT12), and `plan()` refused every other size in both directions. **(2)** The FAT type is decided by `CountofClusters`, never by the `FAT16` string in the BPB — volumes were built landing on exactly 4084, 4085, 65524 and 65525 clusters and `fsck_msdos`, `diskutil` and the writer agree on the type for all four; `buildFat` re-derives the type from the geometry it just wrote and throws rather than emit one it cannot name. **(3)** Every timestamp comes from `determinism.sourceDateEpoch`, packed by its UTC calendar fields, so two cold builds are byte-identical (measured, all three types). vvfat stamped the HOST's clock and took no time option, which cost a staging copy to pin mtime/atime plus a post-hoc rewrite of the creation fields — `st_ctime` is not settable from userspace — and before that rewrite two cold builds seconds apart published **8 differing bytes under one realization key**. None of that machinery remains. What FAT still cannot do is carry a timezone: the Unix time a reader derives is its own, and Darwin's `msdos` driver stats an entry written from `1700000000` back as `1700028800` on a host eight hours behind UTC. |
| Two staged paths differing only in case                                 | **Refused** (`fat-unrepresentable-entry`), at plan time. FAT resolves names case-insensitively, so both entries are written and both names resolve to whichever came first. Measured on macOS 26.5.2 with `a.txt`/`A.txt` and `EFI/BOOT/grub.cfg`/`EFI/BOOT/GRUB.CFG`: `fsck_msdos -n` exits 0 and reports "6 files", the volume mounts, `readdir` lists both names — and opening either name in a pair returns the FIRST entry's bytes. Nothing reports a problem; the second file is simply gone.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
validates a generated FAT against `/sbin/fsck_msdos`, mounts it with Apple's own
`msdos` driver and reads every file back, and puts the generated GPT in front of
`diskutil` — parsers with no qemu code in them. `deno task smoke:fat` does the
same for the FAT writer directly, at both sides of both cluster-count
thresholds. On Linux all of that is skipped, so that leg alone would be qemu
checking qemu. Unit tests run anywhere Deno runs, with no binary at all.

`deno task smoke:block` additionally checks a repaired GPT against macOS's
`diskutil` and `gpt(8)` — parsers with no qemu or package code in them — and
skips silently where they do not exist.
